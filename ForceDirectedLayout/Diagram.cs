...
public class Diagram {
	...
	public void Arrange(double damping, int springLength, int maxIterations, bool deterministic) {
        ...
        int sq = (int)Math.Sqrt(mNodes.Count);
		for (int i = 0; i < mNodes.Count; i++) {
			layout[i] = new NodeLayoutInfo(mNodes[i], new Vector(), Point.Empty);
            bool same = true;
            while (same)
            {
                layout[i].Node.Location = new Point(rnd.Next(-60, 100), rnd.Next(-60, 100));
                for (int j = 0; j < i; j++)
                    if (layout[j].Node.Location == layout[i].Node.Location)
                        continue;
                same = false;
            }
		}
	...
	public void Draw(Graphics graphicsx, Rectangle bounds) {
